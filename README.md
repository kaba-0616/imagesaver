...
Instagramのカルーセルは、表示中のスライドとその前後1枚しかHTML上に存在せず、残りはスワイプして初めて読み込まれます。そのためImageSaverは解析中に「次へ」「前へ」を自動でタップして全スライドを読み込ませています(数秒かかります)。

**この処理は計測のうえ無効化しています。** 4回中2回が「1枚も取得できない」結果になり、成功した回では1枚も増やしていませんでした。送り処理は非同期で完了する必要があり、共有シートがページを覆うとSafariがページのタイマーを凍結することがあるためです(その場合 `completionFunction` が呼ばれず、追加スライドどころか全画像を失います)。

実用上は、**カルーセルの真ん中のスライドを開いてから共有**すれば前後1枚ずつを含めて取得できます(3枚組なら2枚目から共有で全部)。枚数が多い投稿は数回に分けて共有してください。

処理自体は `ImageSaverAction/Action.js` に残してあり、`CAROUSEL_HOSTS` に `"instagram.com"` を戻せば再度有効になります。
