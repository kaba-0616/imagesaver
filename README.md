...
**この処理は現在無効化しています。** 送り処理は非同期で完了する必要がありますが、共有シートがページを覆うとSafariがページのタイマーを凍結することがあり、その場合 `completionFunction` が呼ばれず解析結果が丸ごと失われます(そのページの画像が1枚も出てこない)。追加のスライドどころか全画像を失うため、既定では実行しません。
