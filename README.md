...
この処理はページを操作するため、対象サイトを `ImageSaverAction/Action.js` の `CAROUSEL_HOSTS` で明示的に指定しています(現在はinstagram.comのみ)。対象外のサイトでは一切実行されず、解析は同期的に即座に完了します。

なお送り処理は非同期で完了する必要があり、共有シートがページを覆うとSafariがページのタイマーを凍結することがあります。その場合 `completionFunction` が呼ばれず解析結果が丸ごと失われるため、拡張機能側でそれを検出して「もう一度共有し直してください」と案内します(再共有すればほぼ成功します)。
