...
5. 右上のフィルターアイコンで小さい画像(アイコン等)を除外可能。ページのHTMLソース内にだけ存在する画像(高解像度の原本など)が見つかった場合は、同じメニューの「ソース内の画像も表示」で追加表示できる
6. 「フルスクリーン」表示に切り替えると1枚ずつ大きく確認しながら選択できる(左右スワイプで送り、上下スワイプでグリッドに戻る)
