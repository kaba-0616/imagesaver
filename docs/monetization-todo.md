...
- **原因判明(2026-09-16、build204): 審査提出前はTestFlightでも商品が
  ...
  購入フロー確認はその後**になる。
- **確認済み(2026-09-17、build6): 審査提出後、実機で商品取得・購入フロー
  ともに正常動作。** 初回審査提出(build4/5)を経てTestFlightで
  `Product.products(for:)`が正しくFull/Lite2商品を返すようになり、
  ペイウォールで価格・説明が表示され、購入(Full)も実際に成功して
  「購入状況: Full 有効」に反映された。上記の仮説(審査提出が先、購入
  フロー確認はその後)が実機で裏付けられた形。残タスクは無し、この機能は
  完了。
